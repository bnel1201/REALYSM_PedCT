...
def get_dose(out, logDir='log/'):
    exampleID = 1
    logFile = get_log_name(out, exampleID,logDir=logDir)
...
def evaluate_models_on_log(model_names, FLNAME,DEBUG, NEXAMPLES = 100, dict_dir='', logDir='log/'):
    l_save_dict_name = []
    print("DEBUG ", DEBUG)
...
#             continue
        ...
        l_increasing_dose,l_out = get_data_from_out(FLNAME, NEXAMPLES=NEXAMPLES)
        ...
        for out in l_out:
            try:
                (
                    ...
                    _,
                    num_examples,
                ) = process_output(out, model, num_examples=NEXAMPLES,DOSEID=i, logDir=logDir)
...
def process_output(out, classifier_model, num_examples=50, DOSEID=1,logDir='log/'):
    ############################################################
    ## get accuracy
    l_raw_images0 = []
    l_raw_images = []
...
#         consideredRange = [i for i in range(1, num_examples + 2) if i!=100]
    for exampleID in tqdm(consideredRange):
        logFile = get_log_name(out, exampleID,logDir=logDir)
...
def save_dict(l_outputs, mpath, FLNAME, NEXAMPLES, dict_dir):
    ...
    saveName = get_saveDict_name(FLNAME,mpath,FLOAT, dict_dir)
    print(saveName)
...
def get_saveDict_name(mpath,FLNAME,FLOAT=False, dict_dir=''):
     #"tmp/dicts/"
    saveName = (
        dict_dir
        + FLNAME.split("/")[-1]
...
def get_dict_names(model_names, FLNAME, dict_dir):
    ...
    for mpath in model_names:
        saveName = get_saveDict_name(mpath,FLNAME,FLOAT=False, dict_dir=dict_dir)
        l_save_dict_name.append(saveName)
    return l_save_dict_name

def get_data_from_out(FLNAME, NEXAMPLES=100):
...
def run_dict_script(l_FLNAME, model_runs, dict_dir, logDir='log/',NEXAMPLES=100):
    model_names_all = []
