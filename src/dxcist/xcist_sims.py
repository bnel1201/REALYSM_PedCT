...
from dxcist import DICOM_to_voxelized_phantom
...
def convert_to_dicom(img_slice, phantom_path):
    ...
    ds = dcmread(fpath)
    tmp_slice = np.copy(img_slice) #need to make copy to have side effects on original img_slice outside this function
    tmp_slice += 1024 #check this <-- I shouldnt need this bias
    tmp_slice = tmp_slice.astype('int16')
    ds.Rows, ds.Columns = tmp_slice.shape
    ds.PixelData = tmp_slice.tobytes()
...
def get_reconstructed_data(ct):
    ...
    return xc.rawread(ct.resultsName+f'_{ct.recon.imageSize}x{imsize}x{ct.recon.sliceCount}.raw', [imsize, imsize, ct.recon.sliceCount], 'float')


def run_simulation(ground_truth_image, output_dir, phantom_id='default', slice_id=0, mA=200, kVp=120, FOV=None, mu_water=None):
    ...
    if not FOV:
       FOV = 1.3*get_effective_diameter(ground_truth_image)
